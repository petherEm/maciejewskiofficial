...
export const meta = {
  ...
  description:
    'Yes, I tried GPT-4 model to help me out in some data science tasks. I was curious if it can help me in geo mapping at the large scale.',
}

export default (props) => <ArticleLayout meta={meta} {...props} />


I had to analyse a certain business risk related to geo location of different point of sales. I had a list of addresses and I needed to find out if they are located in the area of interest. I was curious if GPT-4 model can help me out in this task. I was also curious if it can help me in geo mapping at the large scale.
...
<Image src="/blog/distances_1.jpeg" width={800} height={400} alt="" />

I used to do it with different geo coding librabries in the past and this time I decided to ask for quick help the GPT-4 model. I was curious if it can help me out in this task and what would be the suggestion.
...
>This code firstly creates a __KDTree__ for each DataFrame, which is a binary tree structure allowing efficient queries for __nearest neighbours__. Then, for each point in df1, it finds the closest points in df2, df3 and df4 by querying these KDTree structures.

>Please note that the distances are __Euclidean__ and for points on a plane, not for geographic coordinates. If your coordinates span a significant portion of the Earth, this might give incorrect results, since the Earth's surface is not flat. To compute geographical distances accurately, the haversine formula or a library that implements it (like geopy) should be used. But as you requested Euclidean, this solution provides that.
...
>However, since you want the Euclidean distance in meters, __we can do a simple conversion from degrees to meters. One degree of latitude is approximately 111.139 kilometers (which is 111139 meters)__. For longitude, it varies with latitude, but we can use an average value of 111.321 kilometers per degree (which is 111321 meters). But this is still not very accurate.
...
> one more correction - instead of renaming the columns of df1 just add new columns __I am getting lazy with Chat GPT__
